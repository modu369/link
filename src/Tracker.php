            ...
            if ($this->rollupsCoverRange($siteId, $start, $end)) {
                ...
                if (!empty($rollup)) {
                    ...
                    foreach ($rollup as $row) {
                        [$keyword, $engine, $entryHost, $entryPath] = array_pad(explode('|', $row['dimension_value'] ?? '', 4), 4, '');
                        if ($keyword === '') {
                            continue;
                        }
                        $entryPath = $entryPath !== '' ? $entryPath : '/';
                        $entryHost = trim($entryHost);
                        $entryLabel = $entryHost !== '' ? ($entryHost . '/' . ltrim($entryPath, '/')) : $entryPath;
                        ...
                        if (($row['views'] ?? 0) >= ($keywords[$keyword]['views'] ?? 0)) {
                            ...
                            $keywords[$keyword]['entry_host'] = $entryHost;
                            $keywords[$keyword]['entry_path'] = $entryPath;
                        }
                    }
