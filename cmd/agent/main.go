...
func main() {
	...
	agentListen := flag.String("agent-listen", ":9090", "agent control listen")
	agentToken := flag.String("agent-token", "", "agent control token")
	flag.Parse()
	...
	if *agentToken == "" {
		log.Println("warning: agent-token is empty; remote control disabled")
	}
	...
	agentServer := newAgentServer(store, relay, manager, *agentToken)
...
func newAgentServer(store *config.Store, relay *proxy.Proxy, manager *serverManager, token string) http.Handler {
	mux := http.NewServeMux()

	mux.HandleFunc("/agent/config", func(rw http.ResponseWriter, req *http.Request) {
		if !requireAgentAuth(token, rw, req) {
			return
		}
		if req.Method != http.MethodPost {
			rw.WriteHeader(http.StatusMethodNotAllowed)
		...
		var payload control.ConfigPayload
		if err := json.NewDecoder(req.Body).Decode(&payload); err != nil {
			http.Error(rw, "invalid payload", http.StatusBadRequest)
			return
		}
		...
		updated := current
		if payload.ListenHTTP != "" {
			updated.ListenHTTP = payload.ListenHTTP
		}
		if payload.ListenHTTPS != "" {
			updated.ListenHTTPS = payload.ListenHTTPS
		}
		updated.WhitelistDomains = payload.WhitelistDomains
		updated.CloudflareUpstream = payload.CloudflareUpstream
		updated.EnableHTTPProxy = payload.EnableHTTPProxy
		if err := store.Update(updated); err != nil {
			http.Error(rw, "failed to save", http.StatusInternalServerError)
			return
		}
		restarted, err := manager.Update(current, updated)
		if err != nil {
			http.Error(rw, "failed to apply", http.StatusInternalServerError)
			return
		}
		relay.UpdateBackends(updated.CloudflareUpstream)
		relay.UpdateWhitelist(updated.WhitelistDomains)
		response := control.ApplyResponse{
			Status:    "ok",
			Message:   "修改完成",
			Restarted: restarted,
		}
...
func requireAgentAuth(token string, rw http.ResponseWriter, req *http.Request) bool {
	if token == "" {
		http.Error(rw, "agent token not configured", http.StatusForbidden)
		return false
	}
	...
	received := strings.TrimPrefix(auth, "Bearer ")
