...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'update_spider_ips') {
        $newSpiderIps = [];
        foreach ($engines as $key => $name) {
            $newSpiderIps[$key] = trim($_POST["spider_$key"] ?? '');
        }
        $setSetting('spider_ips', $newSpiderIps);
        $spiderIps = $newSpiderIps;
        $message = '蜘蛛 IP 池设置已更新';
        
        // 建议：此处可增加逻辑将配置同步到 Redis，以便 stat.php 高性能读取
        if ($redis) {
            $redis->set('cfg:spider_ips', json_encode($newSpiderIps));
        }
    }
}
...
<div class="sites-layout">
    ...
    <section class="card" style="margin-top: 20px;">
        <div class="section-title">
            <h2>蜘蛛池 IP 库设置</h2>
            <span class="pill">设置各引擎的合法 IP 段（支持 IPv4/IPv6，每行一个），匹配则判定为真蜘蛛</span>
        </div>
        <form method="post" style="display:grid; grid-template-columns:repeat(auto-fit, minmax(300px, 1fr)); gap:16px;">
            <input type="hidden" name="action" value="update_spider_ips">
            
            <?php foreach ($engines as $key => $name): ?>
            <div class="form-control" style="margin:0;">
                <label><?= $name ?> 蜘蛛 IP 段</label>
                <textarea name="spider_<?= $key ?>" rows="5" placeholder="例如：123.125.71.0/24 或 2400:da00::/32"><?= htmlspecialchars($spiderIps[$key] ?? '') ?></textarea>
            </div>
            <?php endforeach; ?>
            
            <div style="grid-column: 1 / -1; text-align: right; margin-top: 10px;">
                <button type="submit">保存蜘蛛 IP 库</button>
            </div>
        </form>
    </section>
</div>
