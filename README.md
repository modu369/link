...
## 功能特性
- 站点注册 / 删除，生成专属 `tracking_id` 并可一键复制埋点脚本。
- JS 埋点脚本，自动上报页面 URL、来源、UA、IP（会进行哈希）、会话页数与停留时长。
- 像素上报接口 `track.php`，兼容无需 JS 的场景。
- 仪表盘展示 PV、UV、IP、平均访问时长/页数、跳出率、近 7 天趋势、热门页面 / 来源、关键词以及最新访问信息。
- 蜘蛛抓取数据单独罗列，不计入正常统计；支持基于历史均值的今日 PV/UV/IP 预估。
